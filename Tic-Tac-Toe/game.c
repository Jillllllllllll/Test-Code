...
void Initialize(char arr[LIN][VER])
{
	...
	for (int i = 0; i < LIN; i++)//用双层for循环让每一个格子为空格
	{
		for (int k = 0; k < VER; k++)
		{
			arr[i][k] = ' ';
...
char IsWin(char arr[LIN][VER])
{
	...
	if (arr[1][1] == arr[0][0] && arr[2][2] == arr[1][1] && arr[1][1] != ' ')//斜着连线的两种情况判断
	{
		return arr[1][1];
	}
